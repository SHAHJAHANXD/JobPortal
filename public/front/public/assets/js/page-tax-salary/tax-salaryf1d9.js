...
function closeToolTip(elementClick) {
...
function clickConverterTimebase() {
    // Validate if we are coming from the converter page and if we don't have parameters in the url
    if (document.referrer.indexOf('convert') > -1 && window.location.href.indexOf('?') === -1) {
        let start = document.referrer.split('start=').pop().split('&')[0]

        // Validate if start parameter from the referrer is not empty, then select timebase button
        if (start) {
            let selector = $("[onclick=\"setBase('timeBasedGroup','per" + start + "',this)\"]");

            // Validate selector, if is not found means we chose semimonthly
            if (selector.length > 0) {
                selector.click();
            } else {
                $("[onclick=\"setBase('timeBasedGroup','persemimonthly',this)\"]").click();
            }
        }
    }
}
...
function eventsListener() {
    ...
    $('#salary-job').autocomplete({
        serviceUrl: '/ajax/auto-suggest.php?type=jobtitle&totalRecords=5&language=' +
